...
class BukuUpdateRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'pengarang' => 'sometimes|required|string|max:100',
            'penerbit' => 'sometimes|required|string|max:100',
            'tahun_terbit' => 'sometimes|required|integer|min:1900|max:' . date('Y'),
            'isbn' => [
                'nullable',
                'string',
                'max:20',
                Rule::unique('bukus', 'isbn')->ignore($this->buku),
            ],
            'jumlah_halaman' => 'nullable|integer|min:1',
            'deskripsi' => 'nullable|string',
            'kategori' => 'sometimes|required|string|max:100',
            'lokasi_rak' => 'sometimes|required|string|max:50',
            'stok' => 'sometimes|required|integer|min:0',
            'dipinjam' => 'sometimes|required|integer|min:0|lte:stok',
